...
router.get("/getAccount", async function (req, res, next) {
  ...
  try {
    if (await sql.checkSessionAndRole(ss, 'getAccount')) {
      ...
      if (typeof req.query.search !== 'undefined' && typeof req.query.searchBy !== 'undefined') {// tính năng tìm kiếm
        // Danh sách các cột có dữ liệu tiếng Việt
        const vietnameseColumns = ['TenNhanVien', 'TenVaiTro'];
      ...
      //sắp xếp 
      result.sort((a, b) => {
        if (sortBy === 'TenNhanVien' || sortBy === 'TenVaiTro') {
